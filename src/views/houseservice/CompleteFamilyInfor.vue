      ...
      <template>
        <div class="md-example-child md-example-child-button md-example-child-button-3">
          <div class="md-example-section-q">
            <md-button type="primary" inline @click="dealSave">确认修改</md-button>
          </div>
        </div>
      </template>
    </footer>
...
import { BItem, BPopCheckList } from "@/components/form";
                ...
                this.basicService.queryHoseHoldPictures(item.id).then(res1 => {
                  if (res1.code === 1) {
                    if (res1.data.length === 0) {
                      ...
                      this.familyCompleteInfo.customerHeaInfoList[index].img =
                        res1.data;
                    }
                  }
                });
