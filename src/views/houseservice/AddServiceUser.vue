...
<div>
  ...
  <div class="content">
    <div class="cue text-666">
      ...
      <div>1、为避免一个用户多次重复服务，创建计划前，请点击
        <span class="check-history text-primary text-nowrap">查看该用户历史入户服务资料</span>
      </div>
    ...
    <form class="">
      ...
      <b-item
        class="br-b mt16"
        title="服务人"
        :value="customerInfo.servicerName"
      >
      </b-item>
      <b-item
...
export default {
  ...
  activated() {
          ...
          if (res.code === 1) {
            if (res.data.orderDetailDtoList.length > 0) {
              res.data.orderDetailDtoList.forEach((item) => {
                this.productNames.push(item.productModel + item.productCategoryName);
    ...
    if (this.$route.params.customerInfo) {
      ...
      if (this.customerInfo.productCategory !== '' && this.customerInfo.productCategory !== null) {
    ...
    if (this.$route.params.accompanyingData) { // 选择随行参与人
  ...
  methods: {
    ...
    getDefaultAddress() {
      this.productService.deafaultCustomerAddress(this.customerInfo.userPhone).then((res) => {
        if (res.code === 1) {
          if (res.data !== null) {
            console.log(res);
            ...
            if (res.data.province) {
              this.getAddressName(res.data.province, res.data.city, res.data.district, res.data.address);
            }
            // 查询客户地址列表
            this.queryCustomerAddressList();
          }
    ...
    selectAddress(item) {
      this.getAddressName(item.province, item.city, item.district, item.address);
    ...
    // 保存
    dealSave() {
      if (this.customerInfo.servicescape === undefined || this.customerInfo.servicescape === '') {
        Toast.failed('入户服务场景不能为空');
        return;
      }
      if (this.customerInfo.serviceTime === undefined || this.customerInfo.serviceTime === '') {
        Toast.failed('计划服务时间不能为空');
        return;
      }
      if (this.customerInfo.servicerName === undefined || this.customerInfo.servicerName === '') {
        Toast.failed('服务人不能为空');
        return;
      } debugger;
      if (this.customerInfo.serviceAddress === undefined || this.customerInfo.serviceAddress === '') {
    ...
    dealTip(id) {
      if (this.isTip) {
        ...
        this.basicService.sellerRemind({
          ...tipInfo
        }, {}).then((res) => {
          if (res.code === 1) {
            debugger;
    ...
    getTipTime(num, type) {
      const startDate = new Date(this.customerInfo.serviceTime.replace(/-/g, '/'));
      let endDate = '';
      if (type === 'd') {
